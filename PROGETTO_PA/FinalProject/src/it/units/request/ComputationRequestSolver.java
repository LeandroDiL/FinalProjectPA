...
import it.units.exceptions.*;
import it.units.server.ServerResponse;
import it.units.server.ServerStatRequestData;


public class ComputationRequestSolver implements ServerResponse {

    public static String solveComputationRequest(String input) {
        try {
            ...
            if (allPartsOfARequest.length >= 3) {
                if (computationKindPartOfTheRequest.equals("COUNT")) {
                    ...
                    return ServerResponse.processOkResponse(processTime, solver.getListSize());
                }
            } else {
